...
- Input view policy: Text leads with formatted object summaries and keeps raw editing below; JSON leads with an epJSON-shaped structured tree and keeps editable epJSON below; Table uses object-type-specific spreadsheet tables with global and per-table orientation controls.
- Readability policy: JSON structured view should avoid generic model/objects wrappers, but it must still look like JSON: object type keys, object name keys, braces, colons, quoted keys, literal values, and compact indentation. Text and Table groups should default open, and table row headers should stay visually consistent.
- Analysis navigation policy: keep right-side result tabs focused on Summary, Schedules, and Systems; analysis rows should locate the matching object in the active input view. Zone summary should grow into a structured navigation surface with surfaces and related objects first.
