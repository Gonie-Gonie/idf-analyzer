...
- Input view policy: Text leads with formatted object summaries and keeps raw editing below; JSON leads with the structured IDF object tree and keeps editable epJSON below; Table uses object-type-specific spreadsheet tables with global and per-table orientation controls.
- Readability policy: JSON structured view should start from IDF objects rather than generic model/objects wrappers, use EnergyPlus object type/name and field labels, and keep indentation compact. Text and Table groups should default open, and table row headers should stay visually consistent.
