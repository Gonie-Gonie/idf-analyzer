...
func TestAnalyzeInputTextIncludesSummary(t *testing.T) {
	...
	if result.Report.Summary.MetricCount != 56 {
		t.Fatalf("summary metric count = %d, want 56", result.Report.Summary.MetricCount)
	}
...
func TestAppAssetHandlerServesSummaryMetricGuides(t *testing.T) {
	...
	if len(guides) != 56 {
		t.Fatalf("summary metric guide API returned %d guides, want 56", len(guides))
	}
}
...
`), 0o644); err != nil {
	...
	if len(result.Metrics) != 56 {
		t.Fatalf("multi summary metrics = %d, want 56", len(result.Metrics))
	}
