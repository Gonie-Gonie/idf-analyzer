...
func TestAnalyzeInputTextIncludesSummary(t *testing.T) {
	...
	if result.Report.Summary.MetricCount != 55 {
		t.Fatalf("summary metric count = %d, want 55", result.Report.Summary.MetricCount)
	}
...
func TestAppAssetHandlerServesSummaryMetricGuides(t *testing.T) {
	...
	if len(guides) != 55 {
		t.Fatalf("summary metric guide API returned %d guides, want 55", len(guides))
	}
}
...
`), 0o644); err != nil {
	...
	if len(result.Metrics) != 55 {
		t.Fatalf("multi summary metrics = %d, want 55", len(result.Metrics))
	}
